"""
使用 Info-Gain 算法的交互式聊天脚本（Dream 模型）。
...
python -u examples/info-gain/dream/chat.py --model_name_or_path "Dream-org/Dream-v0-Instruct-7B" --chat_template False
...
@dataclass
class InfoGainSamplerConfig(InfoGainDreamSamplerConfig):
...
def main():
    ...
    # Info-Gain Dream sampler 需要支持缓存的模型
    # 使用 InfoGainDreamConfig 来加载支持缓存的模型
    from dllm.pipelines.info_gain.dream import InfoGainDreamConfig
    config = InfoGainDreamConfig.from_pretrained(script_args.model_name_or_path)
    model = dllm.utils.get_model(model_args=script_args, config=config).eval()
    
    tokenizer = dllm.utils.get_tokenizer(model_args=script_args)
    
    # 使用 Info-Gain Sampler
    sampler = InfoGainDreamSampler(model=model, tokenizer=tokenizer)
    ...
    print(f"  - Token 温度 (temperature): {sampler_config.temperature}")
    print(f"  - Top-p: {sampler_config.top_p}")
    print(f"  - Top-k: {sampler_config.top_k}")
    print("=" * 80 + "\n")
